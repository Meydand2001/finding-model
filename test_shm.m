...
max_iterations=2000; epsilon=5*10^-5;
...
for s=1:S
    profile on;
    [b_tensor,c_tensors2,iter(s,:),log_likelihood(s,:)] = pmf_est_shm_em(data_set,alphabet,g_alphabet,M,array_of_sizes,fld,epsilon,max_iterations);
    profile off;
    %param_accuracy(s)=param_accuracy_calc(lambda1,b_matrices1,a_tensors1,lambda2,b_matrices2,a_tensors2,M,N);
    pmf2=shm2pmf(b_tensor,c_tensors2,alphabet,M,g_alphabet,N,fld);
