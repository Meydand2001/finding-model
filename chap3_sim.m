...
mre_sat=zeros(S,Tr);
mre_nsat=zeros(S,Tr);
param_accuracy_sat=zeros(S,Tr);
param_accuracy_nsat=zeros(S,Tr);
...
rows = [2000,5000,10000,20000,50000,100000,200000,500000];

for s=1:S
    for tr=1:Tr
        data_set_test_sat=data_set_sat(1:rows(s),:);
        ...
        mre_sat(s,tr) = mre_accuracy_calc(pmf_sat,pmf_est_sat);
        param_accuracy_sat(s,tr) = param_accuracy_calc(lambda_sat,b_matrices_sat,a_tensors_sat,lambda_e_sat,b_matrices_e_sat,a_tensors_e_sat,M_sat,N);

        data_set_test_nsat=data_set_nsat(1:rows(s),:);
        ...
        mre_nsat(s,tr) = mre_accuracy_calc(pmf_nsat,pmf_est_nsat);
        param_accuracy_nsat(s,tr) = param_accuracy_calc(lambda_nsat,b_matrices_nsat,a_tensors_nsat,lambda_e_nsat,b_matrices_e_nsat,a_tensors_e_nsat,M_nsat,N);
